...
public class TxBuilder {
    ...
    public TxBuildResult buildTx(String address, String changeAddress, List<Long> amounts, List<String> addresses, Coin coin) throws TxBuilderException {
        Script scriptPubKey = null;
        scriptPubKey = ScriptBuilder.createOutputScript(address);
        ...
        List<Tx> unspendTxs;
        List<Out> unspendOuts;
        unspendTxs = AbstractDb.txProvider.getUnspendTxWithAddress(address);
        unspendOuts = getUnspendOuts(unspendTxs);
        ...
        Tx emptyWalletTx = emptyWallet.buildTx(address, changeAddress, unspendTxs, prepareTx(amounts, addresses));
        if (emptyWalletTx != null && TxBuilder.estimationTxSize(emptyWalletTx.getIns().size(), scriptPubKey, emptyWalletTx.getOuts(), true) <= BitherjSettings.MAX_TX_SIZE) {
            return new TxBuildResult(emptyWalletTx, TxBuildResult.ResultCode.RESULT_OK);
        } else if (emptyWalletTx != null) {
            throw new TxBuilderException(TxBuilderException.ERR_REACH_MAX_TX_SIZE_LIMIT_CODE);
        }
        ...
        for (long amount : amounts) {
            if (amount < Tx.MIN_NONDUST_OUTPUT) {
                return new TxBuildResult(null, TxBuildResult.ResultCode.ERROR_DUST_OUTPUT);
            }
        }
        ...
        for (TxBuilderProtocol builder : this.txBuilders) {
            Tx tx = builder.buildTx(address, changeAddress, unspendTxs, prepareTx(amounts, addresses));
            if (tx != null && TxBuilder.estimationTxSize(tx.getIns().size(), scriptPubKey, tx.getOuts(), true) <= BitherjSettings.MAX_TX_SIZE) {
                txs.add(tx);
            } else if (tx != null) {
                mayMaxTxSize = true;
            }
        ...
        if (txs.size() > 0) {
            return new TxBuildResult(txs.get(0), -1);
        } else if (mayMaxTxSize) {
            return new TxBuildResult(null, TxBuildResult.ResultCode.ERROR_REACH_MAX_TX_SIZE_LIMIT);
        } else {
...
interface TxBuilderProtocol {
    ...
    public Tx buildTx(String address, String changeAddress, List<Tx> unspendTxs, Tx tx,Coin...coin);
}
...
class TxBuilderEmptyWallet implements TxBuilderProtocol {
    ...
    public Tx buildTx(String address, String changeAddress, List<Tx> unspendTxs, Tx tx,Coin...coin) {
        ...
        if (needMinFee) {
            ...
            // no fee logic
            int s = TxBuilder.estimationTxSize(outs.size(), scriptPubKey, tx.getOuts(), true);
            if (TxBuilder.getCoinDepth(outs) <= TxBuilder.TX_FREE_MIN_PRIORITY * s) {
                fees = Utils.getFeeBase();
            }
        }

        int size = TxBuilder.estimationTxSize(outs.size(), scriptPubKey, tx.getOuts(), true);
