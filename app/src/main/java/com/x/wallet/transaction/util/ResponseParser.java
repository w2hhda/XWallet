...
import com.x.wallet.lib.eth.data.TokenDisplay;
import com.x.wallet.lib.eth.data.TransactionDisplay;
import com.x.wallet.lib.eth.data.WalletDisplay;
...
public class ResponseParser {
    ...
    public static ArrayList<WalletDisplay> parseWallets(String response, ArrayList<String> wallets, Context context) throws Exception {
        ArrayList<WalletDisplay> display = new ArrayList<WalletDisplay>();
        JSONArray data = new JSONObject(response).getJSONArray("result");
        for (int i = 0; i < wallets.size(); i++) {
            BigInteger balance = new BigInteger("0");
            for (int j = 0; j < data.length(); j++) {
                if (data.getJSONObject(j).getString("account").equalsIgnoreCase(wallets.get(i))) {
